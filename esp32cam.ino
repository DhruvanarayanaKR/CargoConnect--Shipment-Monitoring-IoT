...
void configInitCamera() {
  ...
  config.frame_size = FRAMESIZE_SVGA;  // Reduced to SVGA (800x600) to save memory
  config.jpeg_quality = 12;  // Lower quality for smaller image size
  config.fb_count = 1;  // Reduce buffer count to save RAM
...
void loop() {
  ...
  int numNewMessages = bot.getUpdates(bot_lasttime);
  if (numNewMessages > 0) {
...
// Replace with your network credentials
const char* ssid = "OnePlus Nord2 5G";  //WiFi Name
...
// You need to click "start" on a bot before it can message you 
// Initialize Telegram BOT
String chatId = "5957352481";
String BOTtoken = "7068999954:AAGG6eLPA6JqgT58VEUQNgsUvm-EMtPIVLQ";
...
void loop(){
 ...
 if(digitalRead(BUTTON) == LOW){
  ...
  if (millis() - bot_lasttime > BOT_MTBSH)
  {
